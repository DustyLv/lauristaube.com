...
// .wrangler/tmp/pages-9fx4zO/functionsWorker-0.3327363571014026.mjs
...
async function sniffImageType(file) {
  ...
  const at = /* @__PURE__ */ __name2((offset, bytes) => bytes.every((v, i) => b[offset + i] === v), "at");
...
__name2(uploadKeyName, "uploadKeyName");
...
__name2(uploadResponseHeaders, "uploadResponseHeaders");
var PBKDF2_ITERATIONS = 1e5;
function bufToB64(buf) {
  return btoa(String.fromCharCode(...new Uint8Array(buf)));
}
__name(bufToB64, "bufToB64");
__name2(bufToB64, "bufToB64");
...
async function verifyPassword(password, stored) {
  ...
  return constantTimeEqual(bufToB64(bits), hashB64);
}
...
var LOGIN_MAX_FAILS_PER_IP = 5;
var LOGIN_MAX_FAILS_PER_USER = 10;
...
async function loginFailKeys(request, username) {
  const ip = request.headers.get("CF-Connecting-IP") || "unknown";
  const user = String(username || "").trim().toLowerCase();
  return {
    ip: `loginfail:ip:${await sha256Hex(ip)}`,
    user: `loginfail:user:${await sha256Hex(user)}`
  };
}
__name(loginFailKeys, "loginFailKeys");
__name2(loginFailKeys, "loginFailKeys");
async function loginLocked(env, request, username) {
  const keys = await loginFailKeys(request, username);
  try {
    const [ipCount, userCount] = await Promise.all([
      env.KV.get(keys.ip),
      env.KV.get(keys.user)
    ]);
    return Number(ipCount || 0) >= LOGIN_MAX_FAILS_PER_IP || Number(userCount || 0) >= LOGIN_MAX_FAILS_PER_USER;
  } catch (e) {
    return false;
  }
}
__name(loginLocked, "loginLocked");
__name2(loginLocked, "loginLocked");
async function recordLoginFailure(env, request, username) {
  ...
  try {
    ...
    await Promise.all([
      env.KV.put(keys.ip, String(Number(ipCount || 0) + 1), { expirationTtl: LOGIN_LOCK_SECONDS }),
      env.KV.put(keys.user, String(Number(userCount || 0) + 1), { expirationTtl: LOGIN_LOCK_SECONDS })
    ]);
  } catch (e) {
  }
}
__name(recordLoginFailure, "recordLoginFailure");
__name2(recordLoginFailure, "recordLoginFailure");
async function clearLoginFailures(env, request, username) {
  const keys = await loginFailKeys(request, username);
  try {
    await Promise.all([env.KV.delete(keys.ip), env.KV.delete(keys.user)]);
  } catch (e) {
  }
}
__name(clearLoginFailures, "clearLoginFailures");
__name2(clearLoginFailures, "clearLoginFailures");
var SESSION_TTL = 60 * 60 * 24 * 7;
async function createSession(env, userId) {
  const token = crypto.randomUUID() + crypto.randomUUID().replace(/-/g, "");
  await env.KV.put(`session:${token}`, String(userId), { expirationTtl: SESSION_TTL });
  return token;
}
...
async function getSessionUser(env, request) {
  ...
  return await env.KV.get(`session:${token}`) || null;
}
...
__name(secureAttr, "secureAttr");
__name2(secureAttr, "secureAttr");
function sessionCookie(token, request) {
  return `session=${token}; HttpOnly;${secureAttr(request)} SameSite=Strict; Path=/; Max-Age=${SESSION_TTL}`;
}
__name(sessionCookie, "sessionCookie");
__name2(sessionCookie, "sessionCookie");
function clearSessionCookie(request) {
  return `session=; HttpOnly;${secureAttr(request)} SameSite=Strict; Path=/; Max-Age=0`;
}
__name(clearSessionCookie, "clearSessionCookie");
__name2(clearSessionCookie, "clearSessionCookie");
async function checkAuth(request, env) {
  return await getSessionUser(env, request) !== null;
}
__name(checkAuth, "checkAuth");
__name2(checkAuth, "checkAuth");
var text = /* @__PURE__ */ __name2((v) => (typeof v === "string" ? v.trim() : "") || null, "text");
...
__name2(invalidateContent, "invalidateContent");
...
async function onRequestPost(context) {
  ...
  try {
    ...
    if (await loginLocked(env, request, username)) {
      const minutes = Math.round(LOGIN_LOCK_SECONDS / 60);
      return json(
        { error: `Too many failed login attempts. Try again in ${minutes} minutes.` },
        429,
        { "Retry-After": String(LOGIN_LOCK_SECONDS) }
      );
    }
    const row = await env.DB.prepare(
      "SELECT id, password FROM access_keys WHERE username = ?"
    ).bind(username).first();
    ...
    if (!isHashed(row.password)) {
      ...
      await env.DB.prepare("UPDATE access_keys SET password = ? WHERE id = ?").bind(newHash, row.id).run();
    }
    const token = await createSession(env, row.id);
    return json({ success: true }, 200, { "Set-Cookie": sessionCookie(token, request) });
  } catch (err) {
...
async function onRequestGet2(context) {
  const { request, env } = context;
  if (await checkAuth(request, env)) return json({ authenticated: true });
...
async function onRequestGet3(context) {
  ...
  try {
    ...
    if (includeDrafts && !await checkAuth(request, env)) return unauthorized();
    if (!includeDrafts) {
    ...
    const timelineItem = /* @__PURE__ */ __name2(({ section, ...t }) => ({ ...t, resume_bullets: parseList(t.resume_bullets) }), "timelineItem");
...
var isHttpUrl = /* @__PURE__ */ __name2((v) => /^https?:\/\/\S+$/i.test(v || ""), "isHttpUrl");
...
__name(handleSave2, "handleSave2");
__name2(handleSave2, "handleSave");
...
// .wrangler/tmp/bundle-X8jEVu/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-X8jEVu/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.3327363571014026.js.map
